...
public class SolverP2 {
    ...
    public void solve(List<String> allLines) {
        ...
        var updateSize = BigInteger.valueOf(30000000);

        System.out.printf("Total size: %d\n\n", totalSize);
        var freeSpace = BigInteger.valueOf(70000000).subtract(totalSize);
        var neededSpace = updateSize.subtract(freeSpace);
        System.out.printf("it needs to free %d\n\n", neededSpace);

        BigInteger smallestDir = dirSize.values()
                .stream()
                .filter(value -> freeSpace.add(value).compareTo(updateSize) >= 0)
                .min(BigInteger::compareTo).get();
        System.out.printf("Smallest dir size is %d\n", smallestDir);
    }
