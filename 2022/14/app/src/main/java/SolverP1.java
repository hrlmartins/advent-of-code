...
public class SolverP1 {
    public void solve(List<String> allLines) {
        ...
        var lowestPoint = rocksAndSand.stream().max(Comparator.comparingInt(Coordinates::y)).get().y();
        ...
        for (int countSand = 1; ; countSand++) {
            ...
            do {
                ...
                if (!rocksAndSand.contains(positionDown)) { // try move down
                    currentPos = positionDown;
                } else if (!rocksAndSand.contains(positionDownAndLeft)) {
                    currentPos = positionDownAndLeft;
                } else if (!rocksAndSand.contains(positionDownAndRight)) {
                    currentPos = positionDownAndRight;
                }
            } while(!previousPos.equals(currentPos) && currentPos.y() <= lowestPoint);

            // at this point the sand has stopped or started fallen into the abyss.
            if (currentPos.y() >= lowestPoint) {
                // No more sand needed and we need to give the number of sand before since this one is already falling
